...
class SortingRobot:
    ...
    def camera_callback(self, data):
        ...
        try:
            ...
            center_mask = np.zeros_like(mask_red)
            center_mask[center_y1:center_y2, center_x1:center_x2] = 255

            mask_red = cv2.bitwise_and(mask_red, center_mask)
            mask_blue = cv2.bitwise_and(mask_blue, center_mask)
            mask_green = cv2.bitwise_and(mask_green, center_mask)
            mask_black = cv2.bitwise_and(mask_black, center_mask)
            ...
            elif cv2.countNonZero(mask_green) > 3000:
                print(">>> TESPİT: YEŞİL (Kağıt)")
                self.target_box_name = 'kutu_yesil'
                self.detected_type = 'GREEN'
                self.start_sorting_thread()
                
            elif cv2.countNonZero(mask_black) > 3000:
                print(">>> TESPİT: SİYAH (Metal)")
                self.target_box_name = 'kutu_siyah'
                self.detected_type = 'BLACK'
                self.start_sorting_thread()   
